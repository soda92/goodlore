...
const injectStylesAndFonts = () => {
    // Add Google Fonts for all options
    ...
    styleElement.textContent = `
        :root {
            ...
            --font-plus-jakarta: 'Plus Jakarta Sans', var(--font-inter);
            ...
            /* Settings defaults */
            --ui-font-family: var(--font-inter);
            --body-font-family: var(--font-jetbrains);
            --body-font-size: 15px;
...
// Settings Manager (Font Chooser & Size Control)
const initSettings = () => {
    ...
    // 2. Custom Fonts & Size Configuration
    const savedUIFont = localStorage.getItem('goodlore-ui-font') || 'var(--font-inter)';
    const savedBodyFont = localStorage.getItem('goodlore-body-font') || 'var(--font-jetbrains)';
    const savedBodySize = localStorage.getItem('goodlore-body-size') || '15';

    document.documentElement.style.setProperty('--ui-font-family', savedUIFont);
    document.documentElement.style.setProperty('--body-font-family', savedBodyFont);
    document.documentElement.style.setProperty('--body-font-size', `${savedBodySize}px`);

    return {
        uiFont: savedUIFont,
        bodyFont: savedBodyFont,
        bodySize: savedBodySize
    };
};
...
const transformPage = () => {
    ...
    appContainer.innerHTML = `
        <header class="goodlore-header">
            ...
            <div class="goodlore-header-actions">
                ...
                <button class="goodlore-btn goodlore-btn-icon" id="settings-toggle-btn" title="Typography & Settings">
                ...
                <div class="settings-popover" id="settings-popover-panel">
                    <h3 class="settings-section-title">Typography Options</h3>
                    
                    <div class="settings-control-group">
                        <label class="settings-label" for="settings-ui-font-select">UI Font Family</label>
                        <select class="settings-select" id="settings-ui-font-select">
                    ...
                    <div class="settings-control-group">
                        <label class="settings-label" for="settings-body-font-select">Message Font Family</label>
                        <select class="settings-select" id="settings-body-font-select">
                    ...
                    <div class="settings-control-group">
                        ...
                        <div class="settings-slider-row">
                            <input type="range" min="12" max="22" value="${currentSettings.bodySize}" class="settings-slider" id="settings-body-size-slider">
                            <span class="settings-slider-value" id="settings-body-size-val">${currentSettings.bodySize}px</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // Select dropdown options according to stored settings
    document.getElementById('settings-ui-font-select').value = currentSettings.uiFont;
    document.getElementById('settings-body-font-select').value = currentSettings.bodyFont;
    ...
    // 2. Settings popover toggle
    const settingsToggleBtn = document.getElementById('settings-toggle-btn');
    const settingsPopover = document.getElementById('settings-popover-panel');
    ...
    settingsPopover.addEventListener('click', (e) => {
        e.stopPropagation(); // Prevent closing when clicking inside popover
    });
    ...
    // 3. Font and Size inputs change handlers
    const uiFontSelect = document.getElementById('settings-ui-font-select');
    const bodyFontSelect = document.getElementById('settings-body-font-select');
    const bodySizeSlider = document.getElementById('settings-body-size-slider');
    const bodySizeVal = document.getElementById('settings-body-size-val');

    uiFontSelect.addEventListener('change', (e) => {
        const val = e.target.value;
        document.documentElement.style.setProperty('--ui-font-family', val);
        localStorage.setItem('goodlore-ui-font', val);
    });

    bodyFontSelect.addEventListener('change', (e) => {
        const val = e.target.value;
        document.documentElement.style.setProperty('--body-font-family', val);
        localStorage.setItem('goodlore-body-font', val);
    });

    bodySizeSlider.addEventListener('input', (e) => {
        const val = e.target.value;
        bodySizeVal.innerText = `${val}px`;
        document.documentElement.style.setProperty('--body-font-size', `${val}px`);
        localStorage.setItem('goodlore-body-size', val);
    });
    
    // 4. Extra metadata dropdown toggle
    ...
    // 10. Sidebar filter input search list
...
const init = () => {
    ...
    if (document.getElementById('b')) {
        ...
        // Just load basic styling presets for non-thread pages
        initSettings();
    }
};
